...
const ROLE_OPTIONS = [
    { label: 'Quản lý (Manager)', value: 1 },
    { label: 'Nhân viên (Consultant)', value: 2 },
    { label: 'Giáo viên (Teacher)', value: 3 },
    { label: 'Học sinh (Student)', value: 4 },
];
...
const MemberManagement = () => {
    ...
    const handleCreateUser = async () => {
        try {
            ...
            await createUserService(requestData);

            // Thành công thì đóng modal, reset form và tải lại danh sách
            setIsCreateModalOpen(false);
            createForm.resetFields();
            fetchUsers();
        } catch (error) {
            // Lỗi đã được xử lý hiển thị message bên service
        } finally {
